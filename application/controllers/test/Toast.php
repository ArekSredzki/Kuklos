...
abstract class Toast extends MY_Controller
{
	...
	{
		$this->_run_all();
		$data1['modelname'] = $this->modelname;
		$data1['results'] = $this->unit->result();
		$data1['messages'] = $this->messages;

		$output = '';
		$output .= $this->load->view('test/header', NULL, TRUE);
		$output .= $this->load->view('test/results', $data1, TRUE);
		$output .= $this->load->view('test/footer', NULL, TRUE);

		$data['test_output'] = $output;
		$this->template
			->title('XLS', 'Admin', 'Kuklos')
			->build('pages/admin/testing', $data);
	}
